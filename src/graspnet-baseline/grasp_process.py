...
from camera_capture import D435iCapture
...
# ==================== 硬编码配置参数 ====================
CHECKPOINT_PATH = '/home/huang/learn_arm_robot/graspnet-baseline/checkpoint-rs.tar'  # 修改为你的权重路径
NUM_POINT = 10000
NUM_VIEW = 300
COLLISION_THRESH = 0.01
VOXEL_SIZE = 0.01

# 相机内参（Intel RealSense D435i 实际测量值）
DEPTH_INTR = {
    "ppx": 642.15,    # cx (实际测量的主点x坐标)
    "ppy": 356.04,    # cy (实际测量的主点y坐标) 
    "fx": 635.06,     # fx (实际测量的焦距)
    "fy": 635.06      # fy (实际测量的焦距)
}
...
def capture_from_camera():
    ...
    try:
        print("正在启动D435i相机...")
        camera = D435iCapture(width=1280, height=720, fps=30)
        ...
        if color is not None and depth is not None:
            # 保存到capture文件夹
            save_dir = os.path.join(ROOT_DIR, "doc", "capture")
            color_path, depth_path = camera.save_frame(color, aligned_depth, save_dir)
            
            # 始终使用固定的workspace_mask
            mask_path = '/home/huang/learn_arm_robot/graspnet-baseline/doc/example_data/workspace_mask.png'
            
            # 更新相机内参（使用实际的相机内参）
            depth_intrinsics, color_intrinsics = camera.get_camera_intrinsics()
            global DEPTH_INTR
            DEPTH_INTR = {
            ...
            print(f"更新相机内参: fx={DEPTH_INTR['fx']:.2f}, fy={DEPTH_INTR['fy']:.2f}, cx={DEPTH_INTR['ppx']:.2f}, cy={DEPTH_INTR['ppy']:.2f}")
            print(f"使用固定掩码: {mask_path}")
            
            camera.stop()
            return color_path, depth_path, mask_path
        else:
            camera.stop()
            print("采集失败!")
...
if __name__ == '__main__':
    # 首先尝试从相机采集数据
    print("=== 尝试从D435i相机采集数据 ===")
    color_img_path, depth_img_path, mask_img_path = capture_from_camera()
    
    # 如果相机采集失败，使用示例数据
    if color_img_path is None:
        print("\n=== 使用示例数据 ===")
        color_img_path = '/home/huang/learn_arm_robot/graspnet-baseline/doc/example_data/color.png'
        depth_img_path = '/home/huang/learn_arm_robot/graspnet-baseline/doc/example_data/depth.png'
        mask_img_path = '/home/huang/learn_arm_robot/graspnet-baseline/doc/example_data/workspace_mask.png'
    
    print(f"\n使用数据:")
    print(f"  彩色图: {color_img_path}")
    print(f"  深度图: {depth_img_path}")
    print(f"  掩码图: {mask_img_path}")
